...
const Cart = () => {
  const cartJson = localStorage.getItem('cart');
    const carts: any[] = cartJson ? JSON.parse(cartJson) : [];
    const [total,setTotal]=useState(0) 
    useEffect(()=>{
        const total=carts.reduce((acc: number,item: { price: number; quantity: number })=>{
          ...
          carts?.map((cart) => {
              return (
                  <li className="flex items-center gap-4 text-xl shadow-xl p-3 bg-slate-200">
                      ...
                      <div>
                          <h3 className=" text-gray-900 text-xlfont-bold"></h3>
                          ...
                          <dl className="mt-0.5 space-y-px text-[15px] text-gray-600">
                              ...
                              <div>
                                  ...
                                  <dd className="inline text-l">{cart.title}</dd>
                              </div>
                          </dl>
                      </div>
          ...
          <div className="mt-8 flex justify-end border-t border-gray-100 pt-8">
            <div className="w-screen max-w-lg space-y-4">
              ...
              <div className="flex justify-end">
                <span
                  className="inline-flex items-center justify-center rounded-full bg-indigo-100 px-2.5 py-0.5 text-yellow-700"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
